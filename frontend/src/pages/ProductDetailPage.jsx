      ...
      <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="flex gap-2 mt-2">
              {product.images.map((img, i) => (
                <button key={i} onClick={() => setMainImage(i)}
                  className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${i === mainImage ? 'border-blue-800' : 'border-transparent'}`}>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-4">
            {product.discount > 0 ? (
              <>
                <span className="text-3xl font-bold text-blue-800">${parseFloat(product.discount_price).toFixed(2)}</span>
              ...
              <span className="text-3xl font-bold text-blue-800">${parseFloat(product.price).toFixed(2)}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 mt-6">
            {product.stock > 0 ? (
              <>
                <button onClick={() => addToCart(product.id)} className="bg-blue-800 text-white px-8 py-3 rounded-lg hover:bg-blue-900 font-medium">
                  Add to Cart
                </button>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {product.category && (
              <Link to={`/products?category=${product.category.id}`} className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm hover:bg-gray-200">
                {product.category.name}
              </Link>
            )}
            {product.brand && (
              <Link to={`/products?brand=${product.brand.id}`} className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm hover:bg-gray-200">
                {product.brand.name}
              </Link>
            )}
            {product.tags?.map((tag) => (
              <span key={tag.id} className="bg-blue-50 text-blue-800 px-3 py-1 rounded-full text-sm">{tag.name}</span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {related.map((p) => (
              <Link key={p.id} to={`/products/${p.id}`}
                className="bg-white rounded-xl shadow-sm  overflow-hidden hover:shadow-md shadow-gray-200 transition-shadow">
                {p.main_image ? (
                  <img src={p.main_image} alt={p.title} className="w-full h-36 object-cover" />
                ) : (
                  <div className="w-full h-36 bg-gray-200 flex items-center justify-center text-gray-500">No Image</div>
                )}
                <div className="p-3">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {[1, 2, 3, 4, 5].map((r) => (
                <button key={r} type="button" onClick={() => setReviewForm({ ...reviewForm, rating: r })}
                  className={`text-xl ${r <= reviewForm.rating ? 'text-yellow-500' : 'text-gray-700'}`}>★</button>
              ))}
            </div>
            <textarea value={reviewForm.comment} onChange={(e) => setReviewForm({ ...reviewForm, comment: e.target.value })}
              placeholder="Write your review..." rows={3} className="w-full border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-700" required />
            <button type="submit" disabled={reviewSubmitting}
              className="mt-3 bg-blue-800 text-white px-6 py-2 rounded-lg hover:bg-blue-900 disabled:opacity-50 text-sm">
