  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm  p-6 space-y-4">
        {message && <div className={`p-3 rounded-lg text-sm ${message.includes('failed') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
          {message}
        </div>}
        ...
        <div>
          ...
          <input type="text" value={user.email} disabled
            className="w-full border rounded-lg px-4 py-2 bg-stone-50 text-gray-500" />
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <textarea value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })}
              rows={2} className="w-full border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-700" />
          </div>
