  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto mt-20 px-4 text-center">
        <p className="text-gray-700 mb-4">Please login to checkout</p>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm  p-6 space-y-4">
            ...
            <div>
              ...
              <textarea value={form.shipping_address} onChange={(e) => setForm({ ...form, shipping_address: e.target.value })}
                rows={2} className="w-full border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-700" required />
            </div>
            ...
            <button type="submit" disabled={loading || cart.items?.length === 0}
              className="w-full bg-blue-800 text-white py-3 rounded-lg hover:bg-blue-900 disabled:opacity-50 font-medium">
