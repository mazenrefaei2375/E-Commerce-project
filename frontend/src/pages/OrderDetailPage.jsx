...
export default function OrderDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {/* Status */}
      <div className="bg-gray-800 rounded-xl shadow-sm shadow-gray-900 p-6 mb-6">
        ...
        <div className="flex items-center">
          {STATUS_FLOW.map((status, i) => (
            <div key={status} className="flex-1 flex items-center">
              ...
              <span className={`text-xs ml-1 capitalize ${STATUS_FLOW.indexOf(order.status) >= i ? 'text-indigo-600 font-medium' : 'text-gray-500'}`}>
                {status}
              </span>
      ...
      {/* Shipping */}
      <div className="bg-gray-800 rounded-xl shadow-sm shadow-gray-900 p-6">
        <h2 className="font-semibold mb-4">Shipping Details</h2>
        <p className="text-gray-300">{order.shipping_address}</p>
        <p className="text-gray-300">{order.shipping_city}, {order.shipping_country}</p>
      </div>

      {canUpdate && (
        <div className="mt-6 bg-gray-800 rounded-xl shadow-sm shadow-gray-900 p-6">
