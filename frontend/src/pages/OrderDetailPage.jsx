    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center">
          {STATUS_FLOW.map((status, i) => (
            <div key={status} className="flex-1 flex items-center">
              <div className={`w-4 h-4 rounded-full ${STATUS_FLOW.indexOf(order.status) >= i ? 'bg-blue-800' : 'bg-gray-300'}`}></div>
              <span className={`text-xs ml-1 capitalize ${STATUS_FLOW.indexOf(order.status) >= i ? 'text-blue-800 font-medium' : 'text-gray-500'}`}>
                {status}
              </span>
              {i < 3 && <div className={`flex-1 h-0.5 mx-2 ${STATUS_FLOW.indexOf(order.status) > i ? 'bg-blue-800' : 'bg-gray-300'}`}></div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm  p-6 mb-6">
        ...
        <div className="flex justify-between font-bold text-lg pt-4 border-t mt-4">
          <span>Total</span>
          <span className="text-blue-800">${parseFloat(order.total).toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm  p-6">
        ...
        <p className="text-gray-700">{order.shipping_city}, {order.shipping_country}</p>
      </div>
